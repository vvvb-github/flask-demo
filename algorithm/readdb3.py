...
def zhengfaMs(t, ts, RH, u, P):
    # # opt：下面的engine可通过传参engine来进行优化
    # if engine is None:
    #     engine = matlab.engine.start_matlab()
    # evap = engine.demo(matlab.double([data[0]]), matlab.double([data[1]]), matlab.double([data[2]]),
    #                  matlab.double([data[3]]), matlab.double([data[4]]))
    # return evap
    # 风速测量高度
    zu = 12
    # 温度测量高度
...
def readdb3(filepath):
    ...
    for i in range(len(result)):
        if (result[i][0] != None and result[i][1] != None and result[i][2] != None and result[i][3] != None and
            ...
            evaps.append(zhengfaMs(result[i][1], result[i][3], result[i][2], result[i][5], result[i][4]))
    return times, evaps

# times,temperatures,humiditys,temperatureSea,pressures,windspeeds=readdb3('hxywq.db3')
# print(windspeeds)
